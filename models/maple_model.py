...
class MultiFeatureList:
    def __init__(self, levels:list, num_vit_layers=12):
        self.levels = [i if i >= 0 else num_vit_layers + i for i in levels]  # convert negative levels to positive
        self.features = None
        self.counter = 0
    def add_feature(self, value):
        if self.counter in self.levels:
            if self.features is None:
                self.features = value.unsqueeze(1)
            else:
                self.features = torch.cat([self.features, value.unsqueeze(1)], dim=1)
        self.counter += 1
        #self.features.shape = (batch_size, num_levels, feature_dim)

def apply_fv(attn: nn.Module, x: torch.Tensor) -> torch.Tensor:
    """
    对输入 x 仅应用 MultiheadAttention 中的 V 投影（不计算注意力）。
    
    参数:
        attn: nn.MultiheadAttention 实例
        x: 输入张量，形状为 (..., dim)，其中 dim 必须与 attn 的嵌入维度匹配
    
    返回:
        torch.Tensor: V 投影后的张量，形状与 x 相同（最后一维变为 d_model）
    """
    # 检查 attn 是否是 MultiheadAttention
    if not isinstance(attn, nn.MultiheadAttention):
        raise TypeError(f"attn 必须是 nn.MultiheadAttention，实际是 {type(attn)}")
    
    # 提取 V 投影的权重和偏置
    d_model = attn.embed_dim
    v_weight = attn.in_proj_weight[2*d_model:, :]  # 最后 d_model 行是 V 的权重
    v_bias: Optional[torch.Tensor] = None
    
    if attn.in_proj_bias is not None:
        v_bias = attn.in_proj_bias[2*d_model:]     # 最后 d_model 个元素是 V 的偏置
    
    # 确保参数与 x 在同一设备和数据类型上
    v_weight = v_weight.to(x.device).to(x.dtype)
    if v_bias is not None:
        v_bias = v_bias.to(x.device).to(x.dtype)
    
    # 应用线性变换（V 投影）
    # 使用 torch.einsum 或 F.linear 均可，这里选择更通用的 F.linear
    return torch.nn.functional.linear(x, v_weight, v_bias)
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        # self.hierachical = design_details['hierachical'] #only have effect on visual side
        if i == 0:
            self.first_layer = True
        else:
            self.first_layer = False
        self.use_invite = design_details.get('use_invite', False)  # whether to use INViTE or not
        print(f'self.use_invite:{self.use_invite}')

    def attention(self, x: torch.Tensor, need_weights):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0] if not need_weights else self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
    ...
    def forward(self, inputs):
        ...
        compound_prompts_deeper = inputs['compound_prompts_deeper']
        # counter = inputs[2]
        counter = inputs['counter']
        # attn_weights_list = inputs[3]
        attn_weights_list = inputs.get('attn_weights_list', None)
        hfea_list = inputs.get('hfea_list', None)
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # Once done, update the counter, so that the next time, it does not use same learnable tokens
                        counter += 1
        if attn_weights_list is None:
            x = x + self.attention(self.ln_1(x), need_weights=False)
        else:
            delta_x, weights = self.attention(self.ln_1(x), need_weights=True)
            attn_weights_list.append(weights)
            x = x + delta_x
        x = x + self.mlp(self.ln_2(x))
        if hfea_list is None:
            # get normal result
            return {'x':x, 'compound_prompts_deeper':compound_prompts_deeper, 'counter':counter, 'attn_weights_list': attn_weights_list}  # return again as a dict, so that nn.seq can work        
        else:
            # get hierachical result
            assert not self.text_layer #only visual side needs hierachical features
            if self.use_invite:
                # apply INViTE:
                if hfea_list.features is not None:
                    #hfea_list.features.shape (batch_size, cur_num, dim)
                    # # apply residule fv
                    # hfea_list.features = apply_fv(self.attn, self.ln_1(hfea_list.features)) + hfea_list.features
                    # # apply mlp
                    # hfea_list.features = self.mlp(self.ln_2(hfea_list.features)) + hfea_list.features
                    hf = hfea_list.features.transpose(0, 1)
                    # mask = torch.eye(hf.shape[0], device=hf.device) * float("-inf")
                    mask = torch.empty(hf.shape[0], hf.shape[0], device=hf.device)
                    mask.fill_(float("-inf"))
                    mask.fill_diagonal_(0)
                    mask = mask.type(hf.dtype)
                    hf = self.attn(hf, hf, hf, need_weights=False, attn_mask=mask)[0] + hf
                    hf = hf + self.mlp(self.ln_2(hf)) + hf
                    hfea_list.features = hf.transpose(0, 1)

            hfea_list.add_feature(x[0, :, :])
            return {'x':x, 'compound_prompts_deeper':compound_prompts_deeper, 'counter':counter, 'attn_weights_list': attn_weights_list, 'hfea_list':hfea_list}
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, selected_layer_indices=None, hierachical=False, attn_weights_list=None):
        if not hierachical:
            assert selected_layer_indices is None, "selected_layer_indices should be None when hierachical is False"
        ...
        if hierachical:
            # outputs = self.transformer([x, compound_deeper_prompts, 0, MultiFeatureList(selected_layer_indices)])  # third argument is counter, fourth arg is hiefeature list
            outputs = self.transformer({'x':x, 'compound_prompts_deeper':compound_deeper_prompts, 'counter':0, 'hfea_list':MultiFeatureList(selected_layer_indices), 'attn_weights_list':attn_weights_list})  # third argument is counter, fourth arg is hiefeature list
            # import pdb; pdb.set_trace()
            # hie_fea = outputs[3].features # N, levels, D
            hie_fea = outputs['hfea_list'].features # N, levels, D
            hie_fea = self.ln_post(hie_fea)
            if self.proj is not None:
                hie_fea = hie_fea @ self.proj # hie_fea.shape = (N, levels, D), self.proj = (D, out)
            if attn_weights_list is not None:
                return hie_fea, attn_weights_list
            else:
                return hie_fea
        else:
            outputs = self.transformer({'x':x, 'compound_prompts_deeper':compound_deeper_prompts, 'counter':0, 'attn_weights_list':attn_weights_list}) # third argument is counter
            # x = outputs[0]
            x = outputs['x']
            x = x.permute(1, 0, 2)  # LND -> NLD
            x = self.ln_post(x[:, 0, :])
