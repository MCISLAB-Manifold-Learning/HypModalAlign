...
from .maple_model import MultiFeatureList, apply_fv
...
class ResidualAttentionBlock_IVLP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
        ...
        self.use_invite = design_details.get('use_invite', True)  # whether to use INViTE or not
    ...
    def forward(self, inputs):
        ...
        if hfea_list is None:
            # get normal result
            return {'x':x, 'attn_weights_list': attn_weights_list}  # return again as a dict, so that nn.seq can work        
        else:
            ...
            return {'x':x, 'attn_weights_list': attn_weights_list, 'hfea_list':hfea_list}
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, selected_layer_indices=None, hierachical=False, attn_weights_list=None):
        if not hierachical:
        ...
        if hierachical:
            ...
            outputs = self.transformer({'x':x, 'hfea_list':MultiFeatureList(selected_layer_indices), 'attn_weights_list':attn_weights_list})  # third argument is counter, fourth arg is hiefeature list
            ...
            outputs = self.transformer({'x':x, 'attn_weights_list':attn_weights_list}) # third argument is counter
            # x = outputs[0]
            x = outputs['x']
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, selected_layer_indices=None, hierachical=None, attn_weights_list=None):
